...
if not inspector.has_table('hostliste'):
    # Erzeuge die Tabelle in der Datenbank, falls diese noch nicht existiert
    metadata.create_all(engine)
...
def is_valid_mac(mac):
    # Prüft, ob valide MAC-Adresse übergeben wurde (akzeptiert auch Format ohne Doppelpunkte oder Bindestriche)
    mac_pattern = re.compile(r'^([0-9A-Fa-f]{2})([:-]?[0-9A-Fa-f]{2}){5}$')
    return bool(mac_pattern.match(mac))

def show_important_data():
    # Zeigt nur die Daten der erwünschten Spalten (desired_columns)
    hostliste = get_hostliste_from_db()
    if not hostliste.empty:
...
def show_raum_data(raum):
    ...
    if not filtered_data.empty:
        ...
        print(f"Keine Daten für Raum {raum} vorhanden.")

def geraete_in_adressbereich(netzwerkpräfix):
    hostliste = get_hostliste_from_db()
    # Filtern der Daten sodass nur diejenigen Zeilen ausgewählt werden, deren IP-Adresse mit dem Netzwerkpräfix beginnt
    filtered_data = hostliste[hostliste['IP'].str.startswith(netzwerkpräfix)][desired_columns]
    if not filtered_data.empty:
        formatted_data = filtered_data.to_string(index=False)
        print(formatted_data)
    else:
        print(f"Keine Geräte im Adressbereich {netzwerkpräfix} gefunden.")
...
def get_freie_ip(adressbereich):
    ...
    # freie IPs = alle IPs ohne(-) vorhandene IPs
...
def pc_erfassungs_csv_laden():
    ...
    adressbereich = input("Geben Sie den Adressbereich an (Bsp. 10.1.): ")
    for index in range(0, len(df), 2):
        ...
        ip = f"{adressbereich}{int(numerische_raumnummer)}.{''.join(filter(str.isdigit, pc_nummer))}"
